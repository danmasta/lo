import {
    addType,
    from,
    fromEntries,
    getOwnPropertyDescriptor,
    getPrototypeOf,
    hasOwn,
    isPrototypeOf,
    isView,
    noop,
    PRIMITIVES,
    REGEX,
    settings,
    typeByCtor,
    typeByProto,
    typeByType,
    TYPES
} from './constants.js';

// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/toString
function toStringTag (obj) {
    return Object.prototype.toString.call(obj).slice(8, -1);
}

// Get the type definition from an object's prototype
// Will recurse 3 levels to check parent proto if recurse not explicitly false
// Will recurse the whole prototype chain if recurse is -1 or Infinity
// Returns TYPES.Unknown if type not found
export function getTypeFromProto (obj, { recurse=3, of, add=settings.addUnknownTypes, ref=obj }={}) {
    let proto = getPrototypeOf(obj);
    let type = typeByProto(proto);
    if (type) {
        // Note: Cross-realm and null-proto Errors
        if (proto === null && Error.isError(ref)) {
            return TYPES.Error;
        }
        return type;
    }
    // Unknown class or subclass that resolves to object type
    if (proto && of === TYPES.Object && add) {
        return addType({
            c: ref.constructor,
            x: [1, 0, 2, 0]
        });
    }
    if (recurse) {
        recurse--;
        return getTypeFromProto(proto, { recurse, of, add, ref });
    }
    // Note: Fallback (less specificity than checking the proto)
    if (Error.isError(ref)) {
        return TYPES.Error;
    }
    if (type = TYPES[toStringTag(obj)]) {
        return type;
    }
    // Return unknown because null and undefined are technically known types
    return TYPES.Unknown;
}

// Get the type definition from a constructor function
// Will recurse 3 levels to check parent class if recurse not explicitly false
// Will recurse the whole prototype chain if recurse is -1 or Infinity
// Returns TYPES.Unknown if type not found
export function getTypeFromCtor (obj, { recurse=3, of, add=settings.addUnknownTypes, ref=obj }={}) {
    let type = typeByCtor(obj);
    if (type) {
        return type;
    }
    let proto = getPrototypeOf(obj);
    // Unknown class or subclass
    if (isClass(obj) && (proto === TYPES.Function.proto || isClass(proto)) && add) {
        return addType({
            c: ref,
            x: [1, 0, 2, 0]
        });
    }
    // If obj is a constructor fn or class (but not a subclass or extends null)
    // It's prototype will be Function.prototype
    if (proto === TYPES.Function.proto) {
        return TYPES.Function;
    }
    if (recurse) {
        // If obj is a subclass, the constructor prototype will be the parent class:
        // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/extends
        recurse--;
        return getTypeFromCtor(proto, { recurse, of, add, ref });
    }
    // Because obj is a constructor fn we need to check obj.prototype
    if (isPrototypeOf.call(Error.prototype, obj.prototype)) {
        return TYPES.Error;
    }
    if (type = TYPES[toStringTag(obj)]) {
        return type;
    }
    // Return unknown because null and undefined are technically known types
    return TYPES.Unknown;
}

// Get the object type definition
// Uses typeof and constructor/toStringTag fast path for common built-ins,
// otherwise checks the prototype
// Note: Objects that reassign constructor or Symbol.toStringTag opt out of the
// fast path and resolve via the prototype
export function getType (obj) {
    let type = typeByType(typeof obj);
    switch (type) {
        case TYPES.Function:
            if (obj.constructor !== TYPES.Function.ctor) {
                // Known callable subtypes
                // Note: Cross-realm and Function subclasses resolve to Function
                return TYPES[obj[Symbol.toStringTag]] || type;
            }
            return type;
        case TYPES.Number:
            if (Number.isNaN(obj)) {
                return TYPES.NaN;
            }
            if (obj === Infinity || obj === -Infinity) {
                return TYPES.Infinity;
            }
            return type;
        case TYPES.Object:
            if (obj === null) {
                return TYPES.Null;
            }
            // Plain objects (including type definitions)
            if (obj.constructor === type.ctor) {
                return type;
            }
            if (Array.isArray(obj)) {
                return TYPES.Array;
            }
            // Registered prototype objects are plain objects (not instances)
            // Note: Map.prototype includes toStringTag and TypedArray prototypes are undefined
            // Both tag paths misclassify them
            if (typeByProto(obj)) {
                return type;
            }
            // TypedArrays
            // Module
            // Promise
            // Iterator
            // AsyncIterator
            // AsyncFunction
            // GeneratorFunction
            // AsyncGeneratorFunction
            // Generator
            // AsyncGenerator
            if ((type = TYPES[obj[Symbol.toStringTag]]) && type.object) {
                if (type === TYPES.Uint8Array && TYPES.Buffer && obj instanceof TYPES.Buffer.ctor) {
                    return TYPES.Buffer;
                }
                return type;
            }
            return getTypeFromProto(obj, { of: TYPES.Object });
        default:
            return type;
    }
}

// Get the type name string of an object
// returns toStringTag if type not found
export function getTypeStr (obj) {
    let type = getType(obj);
    if (type === TYPES.Unknown) {
        return toStringTag(obj);
    }
    return type.name;
}

// Alias
export function getCtorType (obj) {
    return getTypeFromCtor(obj);
}

// Get the type name string of a constructor function
// returns toStringTag if type not found
export function getCtorTypeStr (obj) {
    let type = getCtorType(obj);
    if (type === TYPES.Unknown) {
        return toStringTag(obj);
    }
    return type.name;
}

// Cast object to a specific type via a constructor or factory function
// Note: Does not validate arguments
export function toType (Type, obj, ...args) {
    let type = getType(obj);
    let ctor = getTypeFromCtor(Type);
    if (type === ctor) {
        return obj;
    }
    switch (ctor.create) {
        case 1:
            return Type.call(undefined, obj, ...args);
        case 2:
            return new Type(obj, ...args);
        default:
            throw new TypeError('Type cannot be called or constructed');
    }
}

// Initialize empty value of same type
export function of (obj, def) {
    let type = getType(obj);
    switch (type.create) {
        case 1:
            return type.ctor.call();
        case 2:
            return new type.ctor();
        default:
            return def;
    }
}

// Reflect.construct throws TypeError if target is not a constructor
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Reflect/construct
export function isCtor (Fn) {
    try {
        Reflect.construct(Fn, []);
        return true;
    } catch (err) {
        if (err instanceof TypeError && err.message.includes('constructor')) {
            return false;
        }
        return true;
    }
}

export function isClass (obj) {
    return isFunction(obj) && getOwnPropertyDescriptor(obj, 'prototype')?.writable === false;
}

export function isNil (obj) {
    return obj == null;
}

export function notNil (obj) {
    return obj != null;
}

export function isNull (obj) {
    return obj === null;
}

export function isUndefined (obj) {
    return obj === undefined;
}

// Test if running in an esm or cjs context
export function isEsm () {
    return typeof module === 'undefined';
}

// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/import#module_namespace_object
export function isModule (obj) {
    return getType(obj) === TYPES.Module;
}

export function isObject (obj) {
    return getType(obj) === TYPES.Object;
}

// Regular or async
export function isFunction (obj) {
    return obj instanceof TYPES.Function.ctor;
}

export function isAsyncFunction (obj) {
    return getType(obj) === TYPES.AsyncFunction;
}

export function isGeneratorFunction (obj) {
    return getType(obj) === TYPES.GeneratorFunction;
}

export function isArray (obj) {
    return getType(obj) === TYPES.Array;
}

export function isPromise (obj) {
    return getType(obj) === TYPES.Promise;
}

// Checks for forEach on prototype
// Array
// Map
// Set
// TypedArray
// Buffer
// Readable
// Transform
// Duplex
// PassThrough
// Iterator
// Generator
// NodeList
export function hasForEach (obj) {
    return getType(obj).each;
}

// Checks for @@iterator
// String
// Array
// Map
// Set
// TypedArray
// Array Iterator
// String Iterator
// Map Iterator
// Set Iterator
// Buffer
// Iterator
// Generator
// NodeList
export function isIterable (obj) {
    return notNil(obj) && !!obj[Symbol.iterator];
}

// Checks for entries on prototype
// Array
// Map
// Set
// TypedArray
// Buffer
// NodeList
export function hasEntries (obj) {
    return getType(obj).entries;
}

// Array
// Map
// Set
// Array Iterator
// String Iterator
// Map Iterator
// Set Iterator
// Iterator
// AsyncIterator
// Generator
// AsyncGenerator
// NodeList
export function isCollection (obj) {
    return getType(obj).collection;
}

export function isIterator (obj) {
    return obj instanceof TYPES.Iterator.ctor
}

export function isAsyncIterator (obj) {
    return TYPES.AsyncIterator.proto.isPrototypeOf(obj);
}

// isView tests the buffer-backed internal slot (includes DataView)
// Note: instanceof alone misfires on TypedArray prototypes
// Ex: Uint8Array.prototype instanceof TypedArray
export function isTypedArray (obj) {
    return isView(obj) && !(obj instanceof TYPES.DataView.ctor);
}

export function isArrayBuffer (obj) {
    return getType(obj) === TYPES.ArrayBuffer;
}

export function isNumber (obj) {
    return getType(obj) === TYPES.Number;
}

export function isNumeric (obj) {
    return !isNaN(parseFloat(obj)) && isFinite(obj);
}

export function isString (obj) {
    return getType(obj) === TYPES.String;
}

export function isBoolean (obj) {
    return getType(obj) === TYPES.Boolean;
}

export function isRegExp (obj) {
    return getType(obj) === TYPES.RegExp;
}

// Checks for @@asyncIterator
// AsyncGenerator
// Currently no built-in async iterables except ReadableStream
export function isAsyncIterable (obj) {
    return notNil(obj) && !!obj[Symbol.asyncIterator];
}

export function isError (obj) {
    return obj instanceof TYPES.Error.ctor
}

function toArrayOrSelf (obj, self) {
    if (isArray(obj)) {
        return obj;
    }
    if (isIterable(obj)) {
        return from(obj);
    }
    return self ? obj : [obj];
}

// Return an array from one or more objects
// If multiple objects are passed they are concatenated into one array
// If an object is iterable it is merged into the array
export function toArray (obj, ...args) {
    if (args.length === 0) {
        return isNil(obj) ? [] : toArrayOrSelf(obj);
    } else {
        args.unshift(obj);
        return Array.prototype.concat.call([], ...args.map(obj => {
            return toArrayOrSelf(obj, true);
        }));
    }
}

export function toFn (obj, fn=noop) {
    return isFunction(obj) ? obj : fn;
}

// Note: Only iterables that implement entries can be cast to an object
// Note: @@iterator only works if [[IteratorKind]] is 'entries'
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/fromEntries
export function toObject (obj) {
    let type = getType(obj);
    if (type === TYPES.Object) {
        return obj;
    }
    if (type.entries) {
        return fromEntries(obj.entries());
    }
    if (type.object) {
        return obj;
    }
    return {};
}

export function toPath (str) {
    if (isArray(str)) {
        return str;
    }
    let arr = toString(str).split(REGEX.path);
    if (!arr.at(0)) {
        arr.shift();
    }
    if (!arr.at(-1)) {
        arr.pop();
    }
    return arr;
}

// Lightweight string coercion
// Note: Mirrors native String() where meaningful
// Note: For more advanced formatting use inspect or inspectLite
export function toString (obj) {
    let type = getType(obj);
    switch (type) {
        case TYPES.String:
            return obj;
        case TYPES.Date:
            return obj.toISOString();
    }
    if (!type.proto) {
        return '';
    }
    // Types with meaningful toString (Number, Boolean, Array, Error, RegExp, Function, Buffer, etc)
    if (type.proto.toString !== Object.prototype.toString) {
        // Note: Built-in prototypes toString method throws on receivers
        // without a matching internal slot
        if (type.object || typeof obj === type.type) {
            return type.proto.toString.call(obj);
        }
    }
    // Render contents for re-iterable collections (Set, Map, NodeList)
    // Note: Skip one-shot iterators/generators
    // Array.from would consume them and hang on infinite generators
    if (type.iterable && !isIterator(obj)) {
        return from(obj).toString();
    }
    return `[object ${type.name}]`;
}

// Convert a string value to it's native type if possible
// Note: Doesn't convert BigInt (Errors with floats and math operations)
// Note: Doesn't convert Symbol
export function toNativeType (val) {
    if (hasOwn(PRIMITIVES, val)) {
        return PRIMITIVES[val];
    }
    if (isNumeric(val)) {
        if (val > Number.MAX_SAFE_INTEGER || val < Number.MIN_SAFE_INTEGER) {
            return val;
        }
        return parseFloat(val);
    }
    return val;
}
