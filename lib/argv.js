...
import { getOwn, hasOwn, isAbsolutePath, split, toCamelCase } from './util.js';
...
// Accepts an optional default argv list, and optional default options
// Returned parseArgv fn accepts an array or string argv list, and optional options
// Supports negation, camel casing, and type casting to native types
// Note: Use quotes for argument values with whitespace
// Note: Either quote style can be used, but mixing quote styles is not supported
// Note: Normalize checks if first argument is an absolute file path, then removes
export function createArgv (defaultArgv=[], defs) {

    function parseArgv (argv, opts) {
        if (isObject(argv)) {
            [argv, opts] = [opts, argv];
        }
        let { argv: optArgv, negate=1, camel=1, native=1, sub='sub', normalize=0 } = { ...defs, ...opts };
        argv ??= optArgv ?? defaultArgv.slice(normalize ? 1 : 2);
        if (!isArray(argv)) {
            argv = split(argv, REGEX.whitespace, { trim: true, quotes: true, extract: true });
        }
        if (normalize) {
            if (argv[0] && isAbsolutePath(argv[0])) {
                argv.shift();
            }
        }
        let res = { _: [] };
    ...
    // Return an options object from parsed argv
    // Accepts an object of key/alias pairs to match values from
    // Note: Optional params are passed to parser if argv is not already parsed
