...
import { isObject, isIterable, isNotNil, isArray, hasForEach, getType, isUndefined, toFn, toObject, isNil } from './types.js';
        ...
        forOwn(obj, (val, key) => {
            if (hasOwn(def, key)) {
                if (isObject(def[key])) {
                    ...
                    if (!hasOwn(res, key) || (isNil(res[key]) && isNotNil(val))) {
                        res[key] = val;
                    }
                }
            }
...
export function assign (res, ...args) {
    ...
    eachNotNil(args, src => {
        forOwn(src, (val, key) => {
            if (!hasOwn(res, key) || isNotNil(val)) {
                res[key] = val;
            }
        });
    });
