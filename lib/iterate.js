...
import { getType, hasForEach, isAsyncFunction, isAsyncIterable, isFunction, isIterable, isNil, notNil, of, toFn } from './types.js';
...
// Note: Can break iteration early by returning BREAK symbol
export function forEach (obj, fn=id) {
    fn = toFn(fn);
    if (hasForEach(obj)) {
        try {
...
// Async alias
async function iterateA (obj, fn=id, col=1, type, iter, iterA, fnA) {
    if (notNil(obj)) {
...
// Note: Can break iteration early by returning BREAK symbol
function iterate (obj, fn=id, col=1, type, iter) {
    if (notNil(obj)) {
        if (col && !type.collection && !type.object) {
            fn(obj, 0, obj);
        } else {
...
// Async alias
async function iterateFA (obj, fn=id, col, retFn, res, valFltr, retFltr, accFn, type, iter, iterA, fnA) {
...
// Note: Can break iteration early by returning BREAK symbol
function iterateF (obj, fn=id, col, retFn, res, valFltr, retFltr, accFn, type, iter) {
...
function compose (obj, fn, col=1) {
    ...
    fn = toFn(fn, id);
    if (async) {
        return iterateA(obj, fn, col, type, iter, iterA, fnA);
    }
...
function composeF (obj, fn, col=1, retFn, res, valFltr, retFltr, accFn) {
    ...
    fn = toFn(fn, id);
    if (async) {
        return iterateFA(obj, fn, col, retFn, res, valFltr, retFltr, accFn, type, iter, iterA, fnA);
    }
    return iterateF(obj, fn, col, retFn, res, valFltr, retFltr, accFn, type, iter);
}

// Run an iterator fn for each item in obj
export function each (obj, fn, col) {
    return compose(obj, fn, col);
}

// Alias for each (ignores null and undefined)
export function eachNotNil (obj, fn, col) {
...
export function reduce (obj, fn, acc, col) {
    return composeF(obj, fn, col, reduceFn, resFn(), undefined, undefined, reduceAcc(acc));
}

// Alias for reduce (ignores null and undefined)
export function reduceNotNil (obj, fn, acc, col) {
    return composeF(obj, fn, col, reduceFn, resFn(), notNil, notNil, reduceAcc(acc));
}
...
export function transform (obj, fn, acc, col) {
...
export function transformNotNil (obj, fn, acc, col) {
...
export function flatMap (obj, fn, recurse, col) {
    return flatMapFn(obj, fn, recurse, col);
}

// Alias for flatMap (ignores null and undefined)
export function flatMapNotNil (obj, fn, recurse, col) {
