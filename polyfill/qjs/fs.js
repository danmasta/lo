...
export function statSync (path, opts) {
    let [stat, err] = os.stat(path);
    if (err) {
        throw new Error(fmt('Stat failed: %s', std.strerror(err)));
    }
...
export function accessSync (path, mode=F_OK) {
    let [stat, err] = os.stat(path);
    if (err) {
        throw new Error(fmt('Access failed: %s', std.strerror(err)));
    }
    if (mode === F_OK) {
        return;
    }
    ...
    throw new Error(fmt('Access failed: %s', std.strerror(std.Error.EACCES)));
}
...
export function mkdirSync (path, mode) {
    ...
    if (err) {
        throw new Error(fmt('Failed to create dir: %s', std.strerror(err)));
    }
}
...
export function openSync (path, flags='r', mode=0o666) {
    ...
    if (fd < 0) {
        throw new Error(fmt('Failed to get fd: %s', std.strerror(fd)));
    }
    return fd;
}
...
export function readSync (fd, buffer, offset, length, position) {
    ...
    if (res < 0) {
        throw new Error(fmt('Failed to read: %s', std.strerror(res)));
    }
    return res;
}
...
export function readdirSync (path) {
    ...
    if (err) {
        throw new Error(fmt('Failed to readdir: %s', std.strerror(err)));
    }
...
// https://github.com/bellard/quickjs/blob/master/quickjs-libc.c#L1623
export function appendFileSync (path, data, { encoding='utf8', mode=0o666, flag='a', flush=false }={}) {
    let err = {};
    let file = std.open(path, flag, err);
    if (err.errno) {
        throw new Error(fmt('Failed to open file: %s', std.strerror(err.errno)));
    }
...
export function readFileSync (path, { encoding=null, flag='r' }={}) {
    let err = {};
    let res;
    let file = std.open(path, flag, err);
    if (err.errno) {
...
// Data can be String, ArrayBuffer, or Uint8Array
// Note: For some reason getting a fd, and using std.fdopen doesn't work
export function writeFileSync (path, data, { encoding='utf8', mode=0o666, flag='w', flush=false }={}) {
    let err = {};
    let file = std.open(path, flag, err);
    if (err.errno) {
        throw new Error(fmt('Failed to open file: %s', std.strerror(err.errno)));
