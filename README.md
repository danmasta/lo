...
## About
I wanted a lighter weight utility library. I had written a bunch of methods over the years that I use between projects as needed, so I decided to normalize them, add a type checking system, and consolidate them into this package. I also wanted to be able to add different methods for environments like node/browser, not have to use polyfills, and support [async iteration](#async-iteration).
...
* `DataView`
* `Stream`
* ...other object types like `RegExp`, `Date`, `Promise`, `Error`, etc
